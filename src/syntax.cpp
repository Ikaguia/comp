...
const std::array<Type, AST::COUNT> AST::types = [] {
	...
	ts[DECLARATION] = Type{DECLARATION, "Declaration", {
		...
		Pattern{Lexic::TYPE::IDENT, Lexic::TYPE::PAOPEN, OP15, Lexic::TYPE::PACLOSE},
		Pattern{Lexic::TYPE::IDENT, Lexic::TYPE::CRLBRKOPEN, NARGS, Lexic::TYPE::CRLBRKCLOSE},
	}};
	...
	ts[IF] = Type{IF, "If", {
		...
		Pattern{Lexic::TYPE::IF, Lexic::TYPE::PAOPEN, DECLARATIONS, Lexic::TYPE::OPSEMICOLON, OP15, Lexic::TYPE::PACLOSE, STATEMENT},
	}};
	...
	ts[WHILE] = Type{WHILE, "While", {
		...
		Pattern{Lexic::TYPE::WHILE, Lexic::TYPE::PAOPEN, DECLARATIONS, Lexic::TYPE::OPSEMICOLON, OP15, Lexic::TYPE::PACLOSE, STATEMENT},
	}};
	...
	ts[DOWHILE] = Type{DOWHILE, "Do While", {
		...
		Pattern{Lexic::TYPE::DO, STATEMENT, Lexic::TYPE::WHILE, Lexic::TYPE::PAOPEN, DECLARATIONS, Lexic::TYPE::OPSEMICOLON, OP15, Lexic::TYPE::PACLOSE, Lexic::TYPE::OPSEMICOLON},
	}};
	...
	ts[STRUCTPROPS] = Type{STRUCTPROPS, "Struct Properties", {
		Pattern{DECLARATIONS, Lexic::TYPE::OPSEMICOLON},
		Pattern{DECLARATIONS, Lexic::TYPE::OPSEMICOLON, STRUCTPROPS},
	}};
...
std::generator<Ranges> gen_ranges(AST::MatchData& data, int start, int end, std::span<const AST::PatternElement> els) {
	...
	// Case: Single non-terminal. It consumes the entire remaining range.
	if (els.size() == 1 && std::holds_alternative<TYPE>(els.front())) {
		co_yield Ranges{Range{start, end}};
		co_return;
	}

	// First element is a Non-Terminal (TYPE)
	if (std::holds_alternative<TYPE>(els.front())) {
		
		// OPTIMIZATION: If the pattern is [Non-Terminal, Terminal, ...], 
		// use the Terminal's positions as anchors instead of trying every split.
		if (els.size() > 1 && std::holds_alternative<Lexic::TYPE>(els[1])) {
			auto tp = std::get<Lexic::TYPE>(els[1]);
			auto& indices = data.indexList[tp];

			// Find all occurrences of the terminal between 'start' and 'end'
			auto it = std::lower_bound(indices.begin(), indices.end(), start);
			while (it != indices.end() && *it < end) {
				int anchor_idx = *it;

				// The first NT range is [start, anchor_idx).
				// We skip the terminal at anchor_idx and recurse on els[2...]
				for (auto sub_ranges : gen_ranges(data, anchor_idx + 1, end, els.subspan(2))) {
					Ranges out;
					out.emplace_back(start, anchor_idx);
			...
			// BRUTE FORCE: First two are Non-Terminals [NT, NT, ...].
			// We must try every possible split point to find a valid match.
	...
	// First element is a Terminal (Lexic::TYPE)
	else if (std::holds_alternative<Lexic::TYPE>(els.front())) {
		auto tp = std::get<Lexic::TYPE>(els.front());
		auto& indices = data.indexList[tp];
		auto it = std::lower_bound(indices.begin(), indices.end(), start);

		// Terminal must be exactly at the current 'start' position
		if (it != indices.end() && *it == start) {
			for (auto sub_ranges : gen_ranges(data, start + 1, end, els.subspan(1))) {
				co_yield sub_ranges;
			}
		}
	}
...
std::set<TYPE> collapsible = {
	AST::OP1, AST::OP2, AST::OP3, AST::OP4, AST::OP5, AST::OP6, AST::OP7, AST::OP8, AST::OP9, AST::OP10, AST::OP11, AST::OP12, AST::OP13, AST::OP14, AST::OP15,
	AST::CODE, AST::BLOCK, AST::STATEMENT, AST::EXPRESSION, AST::PARENTHESIS, AST::VARTYPE, AST::LITERAL, AST::BOOLEAN, AST::NUMBER, AST::ARG, AST::DECLARATIONS,
};
